...
def carregar_pedidos():
    if os.path.exists(FILE_PATH):
        ...
        pedidos = pd.DataFrame(columns=["Nº Pedido", "Fornecedor", "Qtd.", "Valor (R$)", "Pedido por", "Status" "Recebido por", "Tipo Doc", "Nº", "Dt. Receb.", "Hr. Receb.", "Valor Divergente"])
...
def confirmar_recebimento():
    ...
    with st.form("receber_pedido_form"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            valor_correto = st.radio("Valor divergente?", options=["Sim", "Não"])
        with col2:
            valor_incorreto = st.text_input("Se sim, qual valor?")
    ...
    if submit_button:
        if not numero_pedido or not recebido_por or not data_recebimento or not hora_recebimento:
            ...
            st.session_state.pedidos.at[index, "Nº"] = numero_doc
            st.session_state.pedidos.at[index, "Dt. Receb."] = data_recebimento
            st.session_state.pedidos.at[index, "Hr. Receb."] = hora_recebimento
            st.session_state.pedidos.at[index, "Tipo Doc"] = tipo_documento
            st.session_state.pedidos.at[index, "Valor Divergente"] = valor_incorreto
